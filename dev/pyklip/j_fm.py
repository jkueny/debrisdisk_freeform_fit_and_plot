...
def _get_section_indicies(input_shape, img_center, radstart, radend, phistart, phiend, padding, parang, IOWA,
    ...
    if phistart < phiend:
        deltaphi = phiend - phistart + 2 * padding/np.mean(np.asarray([radstart, radend]))
    else:
        deltaphi = (2*np.pi - (phistart - phiend))  + 2 * padding/np.mean(np.asarray([radstart, radend]))
    ...
    if deltaphi >= 2*np.pi:
        ...
        phistart = ((phistart)- padding/np.mean(np.asarray([radstart, radend]))) % (2.0 * np.pi)
        phiend = ((phiend) + padding/np.mean(np.asarray([radstart, radend]))) % (2.0 * np.pi)

    radstart = np.max(np.asarray([radstart-padding,IWA]))
    if OWA is not None:
        radend = np.min(np.asarray([radend+padding,OWA]))
    else:
...
def _save_rotated_section(input_shape, sector, sector_ind, output_img, angle, radstart, radend, phistart, phiend, padding,IOWA, img_center, flipx=True,
    ...
    blank_input_as_sector = blank_input.at[sector_ind].set(sector)
    blank_input_as_sector_reshaped = jnp.reshape(blank_input_as_sector, [dims[0], dims[1]])
    ...
    # rotnans = jnp.where(jnp.isnan(blank_input_as_sector_reshaped[yp_flat_floor, xp_flat_floor]) | 
    #                    jnp.isnan(blank_input_as_sector_reshaped[yp_flat_floor, xp_flat_ceil]) |
    #                    jnp.isnan(blank_input_as_sector_reshaped[yp_flat_ceil, xp_flat_floor]) |
    #                    jnp.isnan(blank_input_as_sector_reshaped[yp_flat_ceil, xp_flat_ceil]))
    ...
    nan_mask = (
        jnp.isnan(blank_input_as_sector_reshaped[yp_flat_floor, xp_flat_floor]) |
        jnp.isnan(blank_input_as_sector_reshaped[yp_flat_floor, xp_flat_ceil])  |
        jnp.isnan(blank_input_as_sector_reshaped[yp_flat_ceil,   xp_flat_floor]) |
        jnp.isnan(blank_input_as_sector_reshaped[yp_flat_ceil,   xp_flat_ceil])
        )
    ...
    # nanpix = jnp.isnan(blank_input_as_sector_reshaped)
    # medval = np.median(blank_input[np.where(~np.isnan(blank_input))])
    # medval = jnp.median(blank_input_as_sector_reshaped[~nanpix])
    # medval = nanmedian(blank_input_as_sector_reshaped)
    input_copy = jnp.copy(blank_input_as_sector_reshaped)
    ...
    output_img_reshape = jnp.reshape(output_img, [outputs_shape[1], outputs_shape[2]])
    # output_img[rot_sector_validpix_2d] = np.nansum([output_img[rot_sector_pix][sector_validpix], rot_sector[sector_validpix]], axis=0)
    output_img_sum = jnp.nansum([output_img[rot_sector_pix][~sector_validpix], rot_sector[~sector_validpix]], axis=0)
